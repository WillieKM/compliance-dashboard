...
export default async function MultiServiceIncidentsPage() {
  const profile = await getCurrentProfile();
  if (!profile) redirect("/login");

  const supabase = await createClient();
  const { data: incidents } = await supabase
    .from("multi_service_incidents").select("*")
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">WAC 388-71-0560 · Document all incidents · DOH notification when required</p>
        </div>
        <Link href="/dashboard/multi-service/incidents/new"
      ...
      {all.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500 mb-4">WAC 388-71-0560 requires documenting all incidents including falls, injuries, and medication errors.</p>
          <Link href="/dashboard/multi-service/incidents/new"
