...
export default function IncidentClearButton({ reportId, clearedBy, clearedAt, supervisorNotes }: Props) {
  ...
  if (clearedBy) {
    return (
      <div className="mt-3 rounded-lg bg-emerald-50 border border-emerald-200 p-3 text-xs text-emerald-800 space-y-1">
        <p className="font-bold text-emerald-900">Cleared by Supervisor</p>
        <p>Supervisor: <strong>{clearedBy}</strong></p>
        {clearedAt && <p>Cleared on: {new Date(clearedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>}
  ...
  return (
    <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3 space-y-2">
      <p className="text-xs font-bold text-slate-700">Supervisor Clearance</p>
      ...
      <textarea
        ...
        rows={2}
        placeholder="Clearance notes (optional)..."
      ...
      <div className="flex gap-2">
        ...
        >
          {loading ? "Clearing…" : "Mark as Cleared"}
        </button>
        <button
