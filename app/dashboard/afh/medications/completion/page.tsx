  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <Link href="/dashboard/afh/medications" className="text-sm hover:underline" style={{ color: amber }}>← Medications</Link>
          <h1 className="text-2xl font-bold text-slate-900 mt-1">eMAR Completion Rate</h1>
          <p className="text-slate-500 text-sm mt-0.5">WAC 388-76-10530 · Weekly med pass compliance by resident</p>
        </div>
          ...
          {dayStats.map(d => {
            ...
            return (
              <div key={d.day} className="flex flex-col items-center gap-1">
                ...
                <span className={`text-xs font-semibold ${isToday ? "text-amber-700" : "text-slate-500"}`}>
                ...
                {isToday && <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-bold">Today</span>}
              </div>
            );
          })}
        </div>
      ...
      {residentStats.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500">No active medications. <Link href="/dashboard/afh/medications/new" className="font-semibold hover:underline" style={{ color: amber }}>Add a medication →</Link></p>
        </div>
      ) : (
