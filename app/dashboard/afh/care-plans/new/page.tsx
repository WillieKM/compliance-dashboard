...
export default async function NewAFHCarePlanPage({ searchParams }: { searchParams: Promise<{ resident_id?: string }> }) {
  ...
  async function save(formData: FormData) {
    ...
    if (planDate) { const d = new Date(planDate); d.setMonth(d.getMonth() + 6); nextReview = d.toISOString().split("T")[0]; }
    let dueDate: string | null = null;
    if (admDate) { const d = new Date(admDate); d.setDate(d.getDate() + 30); dueDate = d.toISOString().split("T")[0]; }
    const resId = String(formData.get("resident_id") || "");
    const res = residents?.find(r => r.id === resId);
    await client.from("afh_care_plans").insert({
    ...
    redirect("/dashboard/afh/care-plans");
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <Link href="/dashboard/afh/care-plans" className="text-sm hover:underline" style={{ color: amber }}>← Care Plans</Link>
        <h1 className="text-2xl font-bold text-slate-900 mt-2">New Individual Care Plan</h1>
        <p className="text-slate-500 text-sm mt-1">WAC 388-76-10415 · Required within 30 days of admission</p>
      </div>
      ...
      <form action={save} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Care Plan Completed Date</label>
          ...
          <div className="sm:col-span-2">
            ...
            <textarea name="services_provided" rows={2} placeholder="e.g. Bathing assistance, meal prep, medication reminders, mobility support..." className={`${inp} resize-none`} />
          </div>
          <div className="sm:col-span-2">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" className="px-8 py-3 rounded-xl text-white font-bold hover:opacity-90" style={{ backgroundColor: amber }}>Save Care Plan</button>
          <Link href="/dashboard/afh/care-plans" className="px-6 py-3 rounded-xl border border-slate-300 font-semibold text-slate-700 hover:bg-slate-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
