...
export default async function AFHVisitsPage() {
  ...
  async function forceClockOut(formData: FormData) {
    ...
    revalidatePath("/dashboard/afh/visits");
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 text-white flex items-center justify-between flex-wrap gap-4"
        style={{ background: `linear-gradient(135deg, ${amber}, #92400e)` }}>
        <div>
          <Link href="/dashboard/afh" className="text-white/70 text-sm hover:text-white mb-1 inline-block">← AFH</Link>
                  ...
                  return (
                    <tr key={v.id} className={`border-b hover:bg-slate-50 ${i % 2 === 0 ? "" : "bg-slate-50/50"}`}>
                      ...
                      <td className="p-3 text-right">
                        <Link href={`/dashboard/afh/visits/${v.id}`} className="text-xs font-semibold hover:underline" style={{ color: amber }}>
                          View →
                        </Link>
                      </td>
                    </tr>
