...
export default async function NewAFHIncidentPage() {
  ...
  async function save(formData: FormData) {
    ...
    await client.from("afh_incidents").insert({
    ...
    redirect("/dashboard/afh/incidents");
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <Link href="/dashboard/afh/incidents" className="text-sm hover:underline" style={{ color: amber }}>← Incident Log</Link>
        <h1 className="text-2xl font-bold text-slate-900 mt-2">Log Incident</h1>
        <p className="text-slate-500 text-sm mt-1">WAC 388-76-10350</p>
      </div>
      <form action={save} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2"><label className="block text-sm font-semibold text-slate-700 mb-1.5">Description</label><textarea name="description" rows={2} placeholder="What happened?" className={`${inp} resize-none`} /></div>
          <div className="sm:col-span-2"><label className="block text-sm font-semibold text-slate-700 mb-1.5">Immediate Action Taken</label><textarea name="immediate_action" rows={2} placeholder="What was done immediately after the incident?" className={`${inp} resize-none`} /></div>
          ...
          <div className="sm:col-span-2 space-y-2">
            <label className={chk}><input type="checkbox" name="witnessed" className="w-5 h-5 accent-amber-600" /><span className="text-sm font-medium text-slate-700">Incident was witnessed by staff</span></label>
            ...
            <label className={chk}><input type="checkbox" name="physician_notified" className="w-5 h-5 accent-amber-600" /><span className="text-sm font-medium text-slate-700">Physician notified</span></label>
            <label className={chk}><input type="checkbox" name="family_notified" className="w-5 h-5 accent-amber-600" /><span className="text-sm font-medium text-slate-700">Family / representative notified</span></label>
            <label className={`${chk} border-red-200 bg-red-50`}><input type="checkbox" name="doh_report_required" className="w-5 h-5 accent-red-600" /><span className="text-sm font-medium text-red-700">DOH report required (24-hour notification)</span></label>
          </div>
        </div>
        <div className="flex gap-3 pt-2">
          <button type="submit" className="px-8 py-3 rounded-xl text-white font-bold hover:opacity-90" style={{ backgroundColor: amber }}>Save Incident</button>
          <Link href="/dashboard/afh/incidents" className="px-6 py-3 rounded-xl border border-slate-300 font-semibold text-slate-700 hover:bg-slate-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
