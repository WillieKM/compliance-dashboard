...
export default async function NewHomeCarePlanPage({ searchParams }: { searchParams: Promise<{ resident_id?: string }> }) {
  ...
  async function save(formData: FormData) {
    ...
    await client.from("home_care_plans").insert({
    ...
    redirect("/dashboard/home-care/care-plans");
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <Link href="/dashboard/home-care/care-plans" className="text-sm hover:underline" style={{ color: navy }}>← Plans of Care</Link>
        <h1 className="text-2xl font-bold text-slate-900 mt-2">New Plan of Care</h1>
        <p className="text-slate-500 text-sm mt-1">WAC 246-335-055 · Required at or before start of services · 60-day review cycle</p>
      </div>
      ...
      <form action={save} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" className="px-8 py-3 rounded-xl text-white font-bold hover:opacity-90" style={{ backgroundColor: navy }}>Save Plan of Care</button>
          <Link href="/dashboard/home-care/care-plans" className="px-6 py-3 rounded-xl border border-slate-300 font-semibold text-slate-700 hover:bg-slate-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
