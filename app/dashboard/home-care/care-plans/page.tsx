...
export default async function HomeCarePlansPage() {
  const profile = await getCurrentProfile();
  if (!profile) redirect("/login");

  const supabase = await createClient();
  const [{ data: plans }, { data: residents }] = await Promise.all([
    supabase.from("home_care_plans").select("*").eq("facility_id", profile.facility_id).order("care_plan_due_date"),
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">WAC 246-335-055 · Required at start of services · Reviewed every 60 days</p>
        </div>
        <Link href="/dashboard/home-care/care-plans/new"
      ...
      {residentsWithout.length > 0 && (
        <div className="rounded-xl border-2 border-amber-200 bg-amber-50 p-4">
          <p className="font-bold text-amber-900 mb-2">⚠ {residentsWithout.length} client{residentsWithout.length > 1 ? "s" : ""} have no plan of care on file</p>
          <div className="flex flex-wrap gap-2 mt-2">
            {residentsWithout.map(r => (
              <Link key={r.id} href={`/dashboard/home-care/care-plans/new?resident_id=${r.id}`}
      ...
      {all.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500 mb-4">WAC 246-335-055 requires a plan of care created and signed before or at the start of services.</p>
          <Link href="/dashboard/home-care/care-plans/new"
