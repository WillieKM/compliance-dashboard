...
export default async function NewSafetyAssessmentPage({ searchParams }: { searchParams: Promise<{ resident_id?: string }> }) {
  ...
  async function save(formData: FormData) {
    ...
    await client.from("home_care_safety_assessments").insert({
    ...
    redirect("/dashboard/home-care/safety-assessments");
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <Link href="/dashboard/home-care/safety-assessments" className="text-sm hover:underline" style={{ color: navy }}>← Safety Assessments</Link>
        <h1 className="text-2xl font-bold text-slate-900 mt-2">New Safety Assessment</h1>
        <p className="text-slate-500 text-sm mt-1">WAC 246-335-055 · Conducted at start of services · Annual review cycle</p>
      </div>
      <form action={save} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-5">
        ...
        <div className="border-t pt-4">
          <h3 className="font-bold text-slate-800 mb-3">Home Environment</h3>
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <label className={chk}><input type="checkbox" name="smoke_detector" className="w-5 h-5 accent-blue-600" /><span className="text-sm font-medium text-slate-700">Working smoke detector present</span></label>
              <label className={chk}><input type="checkbox" name="co_detector" className="w-5 h-5 accent-blue-600" /><span className="text-sm font-medium text-slate-700">Working CO detector present</span></label>
              <label className={chk}><input type="checkbox" name="clear_egress" className="w-5 h-5 accent-blue-600" /><span className="text-sm font-medium text-slate-700">Clear egress / exit routes</span></label>
            </div>
          </div>
        </div>
        ...
        <div className="border-t pt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              ...
              <textarea name="emergency_plan" rows={2} placeholder="e.g. Client calls 911 first, then agency. Neighbor has spare key." className={`${inp} resize-none`} />
            </div>
            <div className="sm:col-span-2">
            ...
            <div className="sm:col-span-2">
              ...
              <textarea name="action_items" rows={2} placeholder="Items to address before or shortly after services begin..." className={`${inp} resize-none`} />
            </div>
          </div>
        </div>

        <div className="flex gap-3 pt-2">
          <button type="submit" className="px-8 py-3 rounded-xl text-white font-bold hover:opacity-90" style={{ backgroundColor: navy }}>Save Assessment</button>
          <Link href="/dashboard/home-care/safety-assessments" className="px-6 py-3 rounded-xl border border-slate-300 font-semibold text-slate-700 hover:bg-slate-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
