...
export default async function HomeSafetyAssessmentsPage() {
  const profile = await getCurrentProfile();
  if (!profile) redirect("/login");

  const supabase = await createClient();
  const [{ data: assessments }, { data: residents }] = await Promise.all([
    supabase.from("home_care_safety_assessments").select("*")
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 mt-1">In-Home Safety Assessments</h1>
          <p className="text-slate-500 text-sm mt-0.5">WAC 246-335-055 · Required at start of services · Annual review</p>
        </div>
        <Link href="/dashboard/home-care/safety-assessments/new"
      ...
      {residentsWithout.length > 0 && (
        <div className="rounded-xl border-2 border-amber-200 bg-amber-50 p-4">
          <p className="font-bold text-amber-900 mb-2">⚠ {residentsWithout.length} client{residentsWithout.length > 1 ? "s" : ""} have no safety assessment on file</p>
          <div className="flex flex-wrap gap-2 mt-2">
            {residentsWithout.map(r => (
              <Link key={r.id} href={`/dashboard/home-care/safety-assessments/new?resident_id=${r.id}`}
      ...
      {all.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500 mb-4">WAC 246-335-055 requires a home safety assessment at the start of services to identify hazards, fall risk, and emergency preparedness.</p>
          <Link href="/dashboard/home-care/safety-assessments/new"
