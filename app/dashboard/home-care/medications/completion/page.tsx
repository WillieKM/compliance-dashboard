  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <Link href="/dashboard/home-care/medications" className="text-sm hover:underline" style={{ color: navy }}>← Medications</Link>
          <h1 className="text-2xl font-bold text-slate-900 mt-1">eMAR Completion Rate</h1>
          <p className="text-slate-500 text-sm mt-0.5">WAC 246-335-065 · Weekly med pass compliance by client</p>
        </div>
        <Link href="/dashboard/home-care/medications/today"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Week Completion",  value: weekPct !== null ? `${weekPct}%` : "—", cls: weekPct !== null && weekPct >= 90 ? "bg-emerald-50 border-emerald-200 text-emerald-800" : weekPct !== null && weekPct >= 75 ? "bg-amber-50 border-amber-200 text-amber-800" : "bg-red-50 border-red-200 text-red-800" },
          ...
          { label: "Clients",          value: residentNames.length,                    cls: "bg-white border-slate-200 text-slate-700" },
        ].map(s => (
          ...
          {dayStats.map(d => {
            ...
            return (
              <div key={d.day} className="flex flex-col items-center gap-1">
                ...
                <span className="text-xs font-semibold" style={{ color: isToday ? navy : undefined }}>
                ...
                {isToday && <span className="text-xs px-1.5 py-0.5 rounded-full font-bold" style={{ backgroundColor: navy + "15", color: navy }}>Today</span>}
              </div>
            );
          })}
        </div>
      ...
      {residentStats.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500">No active medications. <Link href="/dashboard/home-care/medications/new" className="font-semibold hover:underline" style={{ color: navy }}>Add a medication →</Link></p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b" style={{ backgroundColor: navy }}>
            <h2 className="font-bold text-white">By Client — Week of {fmtDate(weekStart)}</h2>
          </div>
              ...
              <div key={r.name} className="p-4">
                <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    ...
                    <span className={`text-sm font-bold ${r.pct === null ? "text-slate-300" : r.pct >= 90 ? "text-emerald-600" : r.pct >= 75 ? "text-amber-600" : "text-red-600"}`}>
                      {r.pct !== null ? `${r.pct}%` : "—"}
                    </span>
                  </div>
                </div>
