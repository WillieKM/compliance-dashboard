...
export default async function LogAdminPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function logAdministration(formData: FormData) {
    ...
    revalidatePath(`/dashboard/home-care/medications/${id}/log`);
  }

  const inp = "w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500";
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        ...
        <form action={logAdministration} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="sm:col-span-2">
            <button type="submit" className="px-6 py-2.5 rounded-lg text-white font-bold text-sm hover:opacity-90" style={{ backgroundColor: navy }}>
              Log Administration
            </button>
          </div>
        </form>
