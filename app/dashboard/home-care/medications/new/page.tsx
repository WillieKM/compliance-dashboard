...
export default async function NewMedicationPage() {
  ...
  async function saveMedication(formData: FormData) {
    ...
    redirect("/dashboard/home-care/medications");
  }

  const inp = "w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500";

  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <Link href="/dashboard/home-care/medications" className="text-sm hover:underline" style={{ color: navy }}>← MAR</Link>
        <h1 className="text-2xl font-bold text-slate-900 mt-2">Add Medication</h1>
        <p className="text-slate-500 text-sm mt-1">WAC 246-335-065</p>
      </div>
      ...
      <form action={saveMedication} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Client Name (if not listed)</label>
          ...
          <div className="sm:col-span-2">
            <label className="flex items-center gap-3 p-3 rounded-xl border-2 border-red-200 bg-red-50 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-xs text-red-700">Requires count log and reconciliation per WAC 246-335-065</p>
              </div>
            </label>
          </div>
          <div className="sm:col-span-2">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link href="/dashboard/home-care/medications" className="px-6 py-3 rounded-xl border border-slate-300 font-semibold text-slate-700 hover:bg-slate-50">
            Cancel
          </Link>
        </div>
      </form>
