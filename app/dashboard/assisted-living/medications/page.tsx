  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <Link href="/dashboard/assisted-living" className="text-sm hover:underline" style={{ color: purple }}>← Assisted Living Dashboard</Link>
          <h1 className="text-2xl font-bold text-slate-900 mt-1">Medication Administration Records</h1>
          <p className="text-slate-500 text-sm mt-0.5">WAC 388-78A-2570 · Active medications and controlled substance log</p>
        </div>
        <div className="flex gap-2">
          <Link href="/dashboard/assisted-living/medications/completion"
            className="px-4 py-2 rounded-lg text-sm font-bold border hover:bg-purple-50"
            style={{ borderColor: purple, color: purple }}>
            📊 Completion Rate
          </Link>
          <Link href="/dashboard/assisted-living/medications/new"
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Active Medications",    count: active.length,       cls: "bg-purple-50 border-purple-200 text-purple-800" },
      ...
      {controlled.length > 0 && (
        <div className="rounded-xl border-2 border-red-200 bg-red-50 p-4">
          ...
          <p className="text-sm text-red-700">WAC 388-78A-2570 requires controlled substance count and reconciliation. Ensure log is up to date.</p>
      ...
      {active.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500 mb-4">Add resident medications to track administration per WAC 388-78A-2570.</p>
          <Link href="/dashboard/assisted-living/medications/new"
                ...
                {active.map((med, i) => (
                  <tr key={med.id} className={`border-b hover:bg-slate-50 ${i % 2 === 0 ? "" : "bg-slate-50/50"}`}>
                    ...
                    <td className="p-3 text-right">
                      <Link href={`/dashboard/assisted-living/medications/${med.id}/log`}
