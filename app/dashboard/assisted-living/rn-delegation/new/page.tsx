...
export default async function NewRNDelegationPage() {
  ...
  async function save(formData: FormData) {
    ...
    redirect("/dashboard/assisted-living/rn-delegation");
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <Link href="/dashboard/assisted-living/rn-delegation" className="text-sm hover:underline" style={{ color: purple }}>← RN Delegation</Link>
        <h1 className="text-2xl font-bold text-slate-900 mt-2">New RN Delegation</h1>
        <p className="text-slate-500 text-sm mt-1">WAC 388-78A-2240</p>
      </div>
      <form action={save} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2">
            <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-slate-200 hover:bg-slate-50">
              <input type="checkbox" name="competency_verified" className="w-5 h-5 accent-purple-600" />
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" className="px-8 py-3 rounded-xl text-white font-bold hover:opacity-90" style={{ backgroundColor: purple }}>Save Delegation</button>
          <Link href="/dashboard/assisted-living/rn-delegation" className="px-6 py-3 rounded-xl border border-slate-300 font-semibold text-slate-700 hover:bg-slate-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
